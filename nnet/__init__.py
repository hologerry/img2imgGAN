...
class Model(object):
    ...
    def generator_input(self, image, z, layers=3, kernels=32, non_lin='lrelu', norm=None,
        ...
        with tf.variable_scope('down_1'):
            conv1 = conv2d(in_layer, ksize=3, out_channels=32, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv1 = conv2d(conv1, ksize=3, out_channels=32, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            pool1 = max_pool(conv1, kernel=2, stride=2, name='pool1')

        with tf.variable_scope('down_2'):
            conv2 = conv2d(pool1, ksize=3, out_channels=64, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv2 = conv2d(conv2, ksize=3, out_channels=64, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            pool2 = max_pool(conv2, kernel=2, stride=2, name='pool1')

        with tf.variable_scope('down_3'):
            conv3 = conv2d(pool2, ksize=3, out_channels=128, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv3 = conv2d(conv3, ksize=3, out_channels=128, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            pool3 = max_pool(conv3, kernel=2, stride=2, name='pool1')

        with tf.variable_scope('down_4'):
            conv4 = conv2d(pool3, ksize=3, out_channels=256, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv4 = conv2d(conv4, ksize=3, out_channels=256, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            pool4 = max_pool(conv4, kernel=2, stride=2, name='pool1')

        with tf.variable_scope('down_5'):
            conv5 = conv2d(pool4, ksize=3, out_channels=512, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv5 = conv2d(conv5, ksize=3, out_channels=512, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)

        with tf.variable_scope('up_1'):
            dcnv1 = deconv(conv5, ksize=3, out_channels=512, stride=2, name='dconv1', out_shape=32,
                           non_lin=self.non_lin[non_lin],
                           batch_size=self.opts.batch_size, reuse=reuse)
            up1 = concatenate(dcnv1, conv4, axis=3)
            conv6 = conv2d(up1, ksize=3, out_channels=256, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv6 = conv2d(conv6, ksize=3, out_channels=256, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)

        with tf.variable_scope('up_2'):
            dcnv2 = deconv(conv6, ksize=3, out_channels=256, stride=2, name='dconv1', out_shape=64,
                           non_lin=self.non_lin[non_lin],
                           batch_size=self.opts.batch_size, reuse=reuse)
            up2 = concatenate(dcnv2, conv3, axis=3)
            conv7 = conv2d(up2, ksize=3, out_channels=128, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv7 = conv2d(conv7, ksize=3, out_channels=128, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)

        with tf.variable_scope('up_3'):
            dcnv3 = deconv(conv7, ksize=3, out_channels=128, stride=2, name='dconv1', out_shape=128,
                           non_lin=self.non_lin[non_lin],
                           batch_size=self.opts.batch_size, reuse=reuse)
            up2 = concatenate(dcnv3, conv2, axis=3)
            conv8 = conv2d(up2, ksize=3, out_channels=64, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv8 = conv2d(conv8, ksize=3, out_channels=64, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)

        with tf.variable_scope('up_4'):
            dcnv4 = deconv(conv8, ksize=3, out_channels=64, stride=2, name='dconv1', out_shape=256,
                           non_lin=self.non_lin[non_lin],
                           batch_size=self.opts.batch_size, reuse=reuse)
            up3 = concatenate(dcnv4, conv1, axis=3)
            conv9 = conv2d(up3, ksize=3, out_channels=32, stride=1, name='conv1', non_lin=self.non_lin[non_lin],
                           reuse=reuse)
            conv9 = conv2d(conv9, ksize=3, out_channels=32, stride=1, name='conv2', non_lin=self.non_lin[non_lin],
                           reuse=reuse)

        with tf.variable_scope('up_5'):
            output = conv2d(conv9, ksize=3, out_channels=3, stride=1, name='conv1', non_lin=self.non_lin['tanh'],
                            reuse=reuse)
    ...
    def discriminator_patch(self, image, kernels, num_layers, norm_layer, non_lin,
        ...
        self.d_layers['conv{}'.format(num_layers + 1)] = conv2d(input_layer, ksize=k, out_channels=1,
                                                                stride=s, name='conv{}'.format(num_layers + 1),
                                                                reuse=reuse)
    ...
    def train(self):
        ...
        for epoch in xrange(0, self.opts.niter + self.opts.niter_decay + 1):
            ...
            for batch_begin, batch_end in zip(xrange(0, self.data.train_size(),
                                              ...
                                              xrange(self.opts.batch_size, self.data.train_size() + 1,
                                                     self.opts.batch_size)):
